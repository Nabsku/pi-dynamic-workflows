...
- Use only `agentType: "analyst"`, `"researcher"`, `"reviewer"`, or `"reporter"`. The provider contract cannot enforce their tool authority, so configure them read-only; use native execution for mutation.
- Do not combine delegation with `schema`, workflow shared-store child tools, workflow custom toolsets, or worktree isolation. Native execution is the sole worktree owner.
- `agentType` is the pi-subagents role. This extension does not merge its local agent definition for delegated calls.
- Delegated model precedence is explicit `model` > resolved `tier` > phase model > metadata model > pi-subagents role/default model. Omit every workflow model route to preserve pi-subagents' configured role/default model.
...
- Delegated and worktree-isolated calls rerun live on workflow resume. Cached text is not evidence that filesystem effects still exist.
