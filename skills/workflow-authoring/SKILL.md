...
- **Delegated backend:** read [pi-subagents backend](references/pi-subagents-backend.md) before using `backend: "pi-subagents"`; it is an explicit fork-only compatibility boundary, not a native-backend replacement.
